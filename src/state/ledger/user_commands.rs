use mina_serialization_types::staged_ledger_diff::{
    self, SignedCommandPayloadBody, StakeDelegation, TransactionStatus,
};
            ...
            .flat_map(|command| {
                ...
                if let TransactionStatus::Applied(_, balance_data_versioned) = command.status.t {
                    ...
                    let (fee_payer, source, receiver) = match command.data.t.t {
                        staged_ledger_diff::UserCommand::SignedCommand(signed_command) => {
                            let fee_payer =
                                signed_command.t.t.payload.t.t.common.t.t.t.fee_payer_pk;
                    ...
                    let fee_payer_balance = balance_data_inner
                        .fee_payer_balance
                        .map(|balance| balance.t.t.t);
                    let receiver_balance = balance_data_inner
                        .receiver_balance
                        .map(|balance| balance.t.t.t);
                    let source_balance = balance_data_inner
                        .source_balance
                        .map(|balance| balance.t.t.t);
                        ...
                        (fee_payer_balance, receiver_balance, source_balance)
                    {
                        ...
                        Some(UserCommand {
                            ...
                            fee_payer: BalanceUpdate {
                                public_key: fee_payer.into(),
                                balance: fee_payer_balance,
                            },
                            source: BalanceUpdate {
                                public_key: source.into(),
