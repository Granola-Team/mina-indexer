...
CREATE TABLE IF NOT EXISTS accounts (
    public_key VARCHAR PRIMARY KEY CHECK (
        public_key SIMILAR TO 'B62[0-9A-Za-z]{52}'
    )
);

CREATE TABLE IF NOT EXISTS blocks (
    hash VARCHAR PRIMARY KEY CHECK (
        hash SIMILAR TO '3N[A-Za-z][0-9A-Za-z]{49}'
    ),
...
CREATE TABLE IF NOT EXISTS epoch_data (
    ...
    epoch_length BIGINT,
    data_type VARCHAR CHECK (data_type IN ('staking', 'next'))
);
...
CREATE TABLE IF NOT EXISTS user_commands (
    ...
    data_type VARCHAR CHECK (data_type IN ('payment', 'staking_delegation')),
    token_id BIGINT,
    amount DECIMAL(38, 9)
);
...
CREATE TABLE IF NOT EXISTS internal_commands (
    ...
    block_hash VARCHAR REFERENCES blocks (hash),
    data_type VARCHAR CHECK (data_type IN ('coinbase', 'fee_transfer')),
...
CREATE TABLE IF NOT EXISTS staking_ledgers (
    ...
    token VARCHAR,
    nonce BIGINT,
    receipt_chain_hash VARCHAR CHECK (
        receipt_chain_hash SIMILAR TO '2[0-9A-Za-z]{51}'
    ),
