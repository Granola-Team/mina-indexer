...
impl StakingLedgerStore for IndexerStore {
    ...
    fn set_staking_account(
        ...
        self.database.put_cf(
            ...
            serde_json::to_vec(&staking_account)?,
        )?;

        // add balance sort data
        self.database.put_cf(
            self.staking_ledger_balance_sort_cf(),
            staking_ledger_sort_key(epoch, staking_account.balance, &pk.0),
            b"",
        )?;
        Ok(())
    }
    ...
    fn add_staking_ledger(
        ...
        for (pk, staking_account) in staking_ledger.staking_ledger.iter() {
        ...
        for (pk, epoch_stake_delegation) in aggregated_delegations.delegations.iter() {
            self.set_epoch_delegations(
                pk.clone(),
                epoch,
                ledger_hash.clone(),
                Some(genesis_state_hash.clone()),
                epoch_stake_delegation,
            )?;
        }
    ...
    fn set_epoch_delegations(
        ...
        trace!("Setting epoch {epoch} stake delegations {pk}");
        let genesis_state_hash = genesis_state_hash.unwrap_or(
            self.get_best_block_genesis_hash()?
                .expect("best block genesis hash exists"),
        );
        self.database.put_cf(
            self.staking_delegations_cf(),
            staking_ledger_account_key(genesis_state_hash, epoch, ledger_hash, pk.clone()),
            serde_json::to_vec(epoch_stake_delegation)?,
        )?;
        self.database.put_cf(
            self.staking_ledger_stake_sort_cf(),
            staking_ledger_sort_key(
                epoch,
                epoch_stake_delegation.total_delegated.unwrap_or_default(),
                &pk.0,
            ),
            b"",
        )?;
                    ...
                    .flatten()
                {
                    if let Some((key_epoch, stake, pk)) = split_staking_ledger_sort_key(&key) {
                        ...
                        let account = self
                            ...
                            .with_context(|| format!("epoch {epoch}, account {pk}"))
                            .expect("staking account exists");
                        let total_delegated = account.total_delegated.expect("total delegated");
    ...
    fn staking_ledger_account_balance_iterator(
        ...
        let rstart = staking_ledger_sort_key(epoch, u64::MAX, "0");
