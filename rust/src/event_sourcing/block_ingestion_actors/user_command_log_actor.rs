...
use crate::event_sourcing::payloads::{BerkeleyBlockPayload, MainnetBlockPayload, UserCommandLogPayload};
...
#[cfg(test)]
mod user_command_log_actor_tests {
    use super::*;
    use crate::event_sourcing::{
        events::{Event, EventType},
        models::{CommandStatus, CommandSummary, CommandType},
        payloads::{BerkeleyBlockPayload, UserCommandLogPayload},
    };
    ...
    async fn test_berkeley_block_actor_handle_event() {
        ...
        // Mock a BerkeleyBlockPayload with user commands
        let user_commands = vec![
            CommandSummary {
                memo: "memo_1".to_string(),
                fee_payer: "payer_1".to_string(),
        ...
        // Capture and verify the published UserCommandLog events
        for user_command in block_payload.user_commands.iter() {
            // Check if the UserCommandLog event was published
            if let Ok(received_event) = receiver.recv().await {
                assert_eq!(received_event.event_type, EventType::UserCommandLog);

                // Deserialize the payload of the UserCommandLog event
                let summary_payload: UserCommandLogPayload = sonic_rs::from_str(&received_event.payload).unwrap();

                // Verify that the UserCommandLogPayload matches the expected values
                assert_eq!(summary_payload.height, block_payload.height);
                assert_eq!(summary_payload.state_hash, block_payload.state_hash);
                assert_eq!(summary_payload.timestamp, block_payload.timestamp);
                assert_eq!(summary_payload.txn_type, user_command.txn_type);
                ...
                assert_eq!(summary_payload.fee_nanomina, user_command.fee_nanomina);
                assert_eq!(summary_payload.amount_nanomina, user_command.amount_nanomina);
                assert_eq!(summary_payload.memo, user_command.memo);
            } else {
                panic!("Did not receive expected UserCommandLog event from UserCommandActor.");
            }
        }
